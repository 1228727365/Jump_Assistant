...
public class HongBaoService extends AccessibilityService {
		...
		switch (idTextTFCount) {
			...
			case 1://id或text数量为1，先查出对应的id或text，然后再查其他条件
				if (tfs.length == 1) {
					...
					if (Toasty.isEmptyArray(listIdText)) {
						break;
					}
	...
	*/
	 ...
	 */
	@RequiresApi(24)
	public void dispatchGestureLongClick(int x, int y, float time) {
		Log.w("time", String.valueOf(time));
		Path path = new Path();
		path.moveTo(x - 1, y - 1);
		path.lineTo(x, y - 1);
		path.lineTo(x, y);
		path.lineTo(x - 1, y);
		dispatchGesture(new GestureDescription.Builder().addStroke(new GestureDescription.StrokeDescription
				(path, 0, (long) time)).build(), null, null);
	...
	public static void recycleAccessibilityNodeInfo(List<AccessibilityNodeInfo> listInfo) {
		if (Toasty.isEmptyArray(listInfo)) return;
